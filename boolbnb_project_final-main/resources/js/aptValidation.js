...
submitForm.addEventListener("submit", function (event) {
    ...
    if (
        ...
        errors.title = `Inserisci un titolo dell\'inserzione che abbia tra i 3 e i 100 caratteri (al momento: ${
            title.value.trim().length
        })`;
    }
    if (description.value.trim().length < 10) {
        errors.description = `Inserisci una descrizione con almeno 10 caratteri. (al momento: ${
            description.value.trim().length
        })`;
    }
    if (
        beds_number.value <= 0 ||
        beds_number.value > 20 ||
        isNaN(beds_number.value)
    ) {
        errors.beds_number = "Inserisci un valore numerico tra 1 e 20";
    }
    if (
        rooms_number.value <= 0 ||
        rooms_number.value > 20 ||
        isNaN(rooms_number.value)
    ) {
        errors.rooms_number = "Inserisci un valore numerico tra 1 e 20";
    }
    if (
        bathrooms_number.value <= 0 ||
        bathrooms_number.value > 20 ||
        isNaN(bathrooms_number.value)
    ) {
        errors.bathrooms_number = "Inserisci un valore numerico tra 1 e 20";
    }
    if (
        square_meters.value < 20 ||
        square_meters.value > 200 ||
        isNaN(square_meters.value)
    ) {
        errors.square_meters = "Inserisci un valore numerico tra 20 e 200";
    }
    if (
        price_per_night.value <= 0 ||
        price_per_night.value > 99999 ||
        isNaN(price_per_night.value)
    ) {
